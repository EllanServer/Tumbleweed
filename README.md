...
| `plugins/ModelEngine/blueprints/tumbleweed.bbmodel` | 3D 模型蓝图 |
| `plugins/ModelEngine/textures/tumbleweed.png` | 模型纹理 |

3. 若导出被关闭(`config.yml` 中 `auto-export-resources: false`),
   可手动从 jar 内提取 `mythicmobs/`、`modelengine/` 目录按上述路径放置。
4. 服务器控制台执行 `/mm reload` 加载怪物配置,执行 `/me reload` 加载模型蓝图。
...
- 血量 1、无 AI、免疫摔落/火焰/溺水/爆炸;原猪实体隐形,渲染由 ModelEngine 完成。
