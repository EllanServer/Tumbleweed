...
culling:
  enabled: true              # 是否启用 CE 可见性剔除
  near-distance: 32          # 距最近玩家该距离(格)内始终全速,不做剔除
  far-distance: 96           # 距最近玩家超过该距离(格)按距离降频,不做剔除
                             # 区间内(near ~ far)查询 CE 可见性,不可见则降频
```
...
4. **CraftEngine 可见性剔除(可选)**:安装 CraftEngine 后(未安装自动降级,
   行为等同仅距离判定),对距最近玩家 32~96 格的风滚草,每 10 tick 通过
   `EntityCulling.isVisible` 判断该玩家视角是否可见(视锥 + 遮挡);
   不可见的风滚草进入降频循环(物理与渲染同步降频),玩家转头看见时立即恢复全速。
   距离配置见 `config.yml` 的 `culling` 段(`culling-enabled` / `culling-near-distance` /
   `culling-far-distance`,默认 32 / 96);超过远距离的风滚草仍按距离降频。
